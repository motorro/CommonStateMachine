...
## Data flow
Take the created [Gestures and UI-states](gesture-and-ui-state-api.md), [input and result](data-and-ui-api.md) and create an interface or a typealias
to bind the generics of the `CommonFlowDataApi`. This API would be available to the flow consumer to start the feature flow.
...
to bind the generics of the `CommonFlowUiApi`. This API would be available to the flow consumer to display the feature.
Take a look at the [class diagram](../assets/common-child-flow.puml) for more details.
