...
package com.motorro.statemachine.lifecycle
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            // Watch view lifecycle
            val viewLifecycle = LocalLifecycleOwner.current.lifecycle
            DisposableEffect(key1 = viewLifecycle) {
                val observer = model.bindLifecycle(viewLifecycle)
                onDispose {
                    observer.dispose()
                }
            }
